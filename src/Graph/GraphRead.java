...
public class GraphRead {
	 ...
	 * 
	 * @param dat         a file with values in the following format: 1. row: number
	 *                    of vertices (n) 2. row: number of edges (m) only if
	 *                    standardIds = false: The subsequent n rows: the ids of the
	 *                    vertices and the name of the vertices, separated by a
	 *                    blank The subsequent m rows: start- and endvertex of the
	 *                    edge and the weight of the edge (only if weighted = true),
	 *                    separated by blanks
	 * @param directed    true iff the graph is directed in the case directed =
	 *                    true: each edge e = (a,b) in dat is only added to the
	 *                    adjacency list of a in the case directed = false: each
	 *                    edge e = (a,b) is added into the adjacency list of a and
	 *                    in the adjacency list of b
	 * @param standardIds true if the vertices have ids between 0 and n-1 and no
	 *                    names
	 * @param weighted    true if the edges are weighted
	 * @return the graph
	 */
	public static Graph<Vertex, Edge<Vertex>> FileToGraph(String dat, boolean directed, boolean standardIds,
			boolean weighted) {
