...
/**
 * A class which represents the data structure graph; internally the graph is
...
public class Graph<V extends Vertex, E extends Edge<V>> {
	/**
	 * maps a vertex-id to the index, where the corresponding vertex is positioned
	 * in the list vertices and the incident edges are positioned in adjList
	 */
	private HashMap<Integer, Integer> vertexIndex;

	/**
	 * a list with all vertices of the graph; notice, if a vertex is removed from
	 * the graph then the corresponding entry in vertices is set to null
	 */
	private ArrayList<V> vertices;

	/**
	 * a list with all incident edges of every vertex; if a vertex v is positioned
	 * at index i, then the adjacency list of v is positioned at index i, too
	 */
	private ArrayList<LinkedList<E>> adjList;

	/**
	 * true iff graph is directed; notice, an edge in a undirected graph is
	 * contained in the adjacency lists of both vertices (a and b) of the edge
	 */
	 ...
	 * 
	 * @param vertexset  the set of vertices of the graph with pairwise different
	 *                   id's
	...
	public Graph(Collection<V> vertexset, Collection<E> edgeset, boolean isDirected) {
		...
		for (V v : vertexset) {
			...
			if (idx != null)
				throw new RuntimeException("dublicated vertex-id");
	...
	public Collection<E> getEdges() {
		...
		for (V v : vertices) {
			...
			for (E e : neighborEdges) {
				// if graph is not directed, only edges in the neigbor list of "start vertex" is
				// considered
	 ...
	 * @return true iff vertex was inserted, that is, no other vertex with same id
	 *         with v exists
	 */
	 ...
	 * @return true, iff edge was inserted (that is the vertices of the edge are
	...
	public boolean addEdge(E e) {
		...
		int idB = b.getId();
		/*
		 * determines if the vertices of the edge are contained in the vertexset of the
		 * graph
		 */
	...
	public boolean removeVertex(V v) {
		...
		// remove vertex v from ArrayList vertices by setting the corresponding entries
		// to null
	...
	public String toString() {
		StringBuffer s = new StringBuffer(
				"Graph with " + this.noVertices + " vertices; isDirected: " + isDirected + "\n");
		int i = 0;
		for (V v : vertices) {
			if (v != null) {
				s.append(v + ": ");
